...
func init() {
	driver.PaymentRegister(channel.Channel_Wechat, Payment{})
	driver.RefundRegister(channel.Channel_Wechat, Refund{})
}
...
func (p Payment) Open(c config.Config) (iface.Pay, error) {
	...
	return nil, errors.ErrorParamError("wechat-payment: unknown payment product")
}
...
func (p Payment) IsSupportPayment(product payment.PaymentProduct, device enum.Device) bool {
	...
	case payment.PaymentProduct_JSAPI:
		switch device {
		case enum.Device_Wechat, enum.Device_Wechat_Lite:
			return true
		}
	case payment.PaymentProduct_LITE:
		switch device {
	...
	return []iface.PaymentMethod{
		{
			...
			Product: []iface.PaymentProduct{
				...
				{
					Product: payment.PaymentProduct_APP.String(),
					Label:   "APP支付",
				},
			},
		},
